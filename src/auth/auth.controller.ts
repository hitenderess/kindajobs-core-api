...
import { LoginCredentialDto, RefreshTokenDto } from './auth.dto';
...
export class AuthController {
    ...
    @Post('provider/login')
    async Login(@Body() input: LoginCredentialDto) {
    ...
    async providerLogout(@CurrentUser() user: { sub: string; aud: string }, @Body() input: LatLngDto){
        await this.authService.logout(user.sub, user.aud.toUpperCase() as UserType, input)
    }
    ...
    async permissions(@CurrentUser() user: { sub: string; aud: string }): Promise<any> {
        return this.authService.getPermission(user.aud.toUpperCase() as UserType);
    }
